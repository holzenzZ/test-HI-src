...
int status_calc_mob_(struct mob_data* md, enum e_status_calc_opt opt)
{
	...
	if(flag&4) { // Strengthen Guardians - custom value +10% / lv
		...
		if (!gc)
			ShowStatus("status_calc_mob: castle set at map %s\n", map[md->bl.m].name);
...
int status_calc_pc_sub(struct map_session_data* sd, enum e_status_calc_opt opt)
{
	...
	//======= RepBonus =======
	npcd = npc_name2id("RankStatus"); //ชื่อต้องตรงกับใน npc script
	if(sd && npcd){
...
static short status_calc_fix_aspd(struct block_list *bl, struct status_change *sc, int aspd)
{
	...
	if (map_flag_gvg(bl->m) || map_flag_vs(bl->m)) {
		if (sc->data[SC_DONTFORGETME])
			aspd += sc->data[SC_DONTFORGETME]->val2;
	}
...
// (^~_~^) Auras End
	...
	switch (type) {
		...
		case SC_AUDIENCE:
			...
			sd->base_status.speed = cap_value(1, MIN_WALK_SPEED, MAX_WALK_SPEED);

			if (sd->base_status.speed != DEFAULT_WALK_SPEED) {
				sd->state.permanent_speed = 1;
			}
